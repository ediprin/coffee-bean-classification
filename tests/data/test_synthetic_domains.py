...
from bilinear_lmmd.data.preparation.prepare_synthetic_domains import prepare_synthetic_domains
from bilinear_lmmd.experiments.run_synthetic_benchmark import (
    METRICS,
    _collect_summary,
...
def test_source_checkpoint_spec_and_validation_are_seed_aware(tmp_path):
    ...
    config = load_config("configs/coffee17/M0_mobilenetv3_gap_source.yaml")
    config["seed"] = 123
...
def test_source_checkpoint_validation_rejects_partial_history(tmp_path):
    ...
    checkpoint_path.parent.mkdir()
    config = load_config("configs/coffee17/M1_mobilenetv3_hbp_source.yaml")
    config["seed"] = 123
    torch.save({"config": config}, checkpoint_path)
    (checkpoint_path.parent / "history.json").write_text(
...
def test_m5w01_rescue_changes_only_adaptation_weight_and_output():
    baseline = load_config("configs/coffee17/M5_mobilenetv3_hbp_lmmd.yaml")
    rescue = load_config("configs/coffee17/M5w01_mobilenetv3_hbp_lmmd_w01.yaml")
