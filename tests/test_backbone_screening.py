...
from bilinear_lmmd.run_backbone_screening import BACKBONES, _selected_models
...
@pytest.mark.parametrize("family", tuple(EXPECTED))
@pytest.mark.parametrize("head", ("gap", "hbp"))
def test_backbone_model_forward_shape(family: str, head: str) -> None:
    ...
    expected_embedding = 1536 if head == "hbp" else model.pool.output_dim
