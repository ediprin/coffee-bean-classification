...
from bilinear_lmmd.data import _transforms
from bilinear_lmmd.prepare_paper_protocol import prepare_paper_protocol
from bilinear_lmmd.run_paper_reproduction import _model_counts
...
def test_paper_configs_freeze_backbone_and_differ_only_in_pooling_capacity():
    gap = load_config("configs/P0_paper_mobilenetv3_gap_ce.yaml")
    hbp = load_config("configs/P1_paper_mobilenetv3_hbp_ce.yaml")
    gap_total, gap_trainable = _model_counts(
        Path("configs/P0_paper_mobilenetv3_gap_ce.yaml")
    )
    hbp_total, hbp_trainable = _model_counts(
        Path("configs/P1_paper_mobilenetv3_hbp_ce.yaml")
    )
