...
class AF2Config:
    """Frozen patchwise angular-frequency contract derived from AFAB-2."""
...
def af2_entropy_threshold(
    probability: torch.Tensor, gamma: float, eps: float = 1e-8
) -> torch.Tensor:
...
class AF2Frontend(nn.Module):
    ...
    def _build_angle_bins(patch_size: int, angular_bins: int) -> torch.Tensor:
        ...
        return (
            torch.floor(degrees / (360.0 / angular_bins))
            .long()
            .clamp_(0, angular_bins - 1)
        )
    ...
    def _af2_weight(self, shifted_frequency: torch.Tensor) -> torch.Tensor:
        ...
        index = (
            self.angle_bin.to(device=magnitude.device)
            .reshape(1, 1, -1)
            .expand(n, c, -1)
        )
        ...
        threshold = af2_entropy_threshold(
            probability, self.config.gamma, self.config.eps
        )
    ...
    def _recover_one(self, image: torch.Tensor) -> torch.Tensor:
        ...
        divisor = F.fold(
            ones, output_size=(hp, wp), kernel_size=m, stride=stride
        )
    ...
    def recover(self, value: torch.Tensor) -> torch.Tensor:
        if value.ndim != 4:
            raise ValueError("AFAB-2 membutuhkan BCHW")
        if not torch.is_floating_point(value):
            raise TypeError("AFAB-2 membutuhkan tensor floating point")
        return torch.cat(
...
class AF2LuminanceFrontend(nn.Module):
    """AFAB-2 cue from Rec.709 luminance, shared across untouched RGB."""

    def __init__(self, config: AF2Config | dict[str, Any] | None = None) -> None:
        super().__init__()
        self.config = AF2Config.from_mapping(config)
        self.af2 = AF2Frontend(self.config)

    def recover_luminance(self, value: torch.Tensor) -> torch.Tensor:
        luminance = rec709_luminance(value)
        return self.af2.recover(luminance)

    def shared_gate(self, value: torch.Tensor) -> torch.Tensor:
        return minmax_spatial(
            self.recover_luminance(value), eps=self.config.eps
        )

    def forward(self, value: torch.Tensor) -> torch.Tensor:
        if value.ndim != 4 or value.shape[1] != 3:
            raise ValueError("AF2 luminance membutuhkan BCHW RGB")
        if not torch.is_floating_point(value):
            raise TypeError("AF2 luminance membutuhkan tensor floating point")
