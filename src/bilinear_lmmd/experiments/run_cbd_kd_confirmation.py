...
from bilinear_lmmd.experiments.run_cbd_stacking_confirmation import _metrics, load_prediction_table


KD_CONFIGS = {
    "GAP_KD_CONTROL": (
        Path("configs/cbd/CBD4_mobilenetv3_gap_kd_gapcal.yaml"),
        "gap_cal",
    ),
    "STACKING_KD": (
        Path("configs/cbd/CBD5_mobilenetv3_gap_kd_stacking.yaml"),
        "stacking",
    ),
}
        ...
        for code, (config_path, teacher_kind) in KD_CONFIGS.items():
            ...
            if _training_complete(run_dir, epochs):
                ...
                command = [
                    ...
                    "-u",
                    "-m",
                    "bilinear_lmmd.experiments.train_cbd_distillation",
                    "--config",
                    str(config_path),
                    "--teacher",
                    teacher_kind,
