...
def run_at_sbn(
    *,
    ...
    # Probe only the deployable base. The auxiliary heads must not change the
    # paired primary initialization contract.
    seed_everything(42)
    ...
    # Also assert the wrapper preserves that exact primary initialization.
    seed_everything(42)
    wrapped_probe = AuxiliaryTrainingModel(copy.deepcopy(cfg["model"]))
    if wrapped_probe.primary_initial_sha256 != expected_initial:
        raise RuntimeError("AT-SBN wrapper mengubah primary initialization.")
    del wrapped_probe
    ...
    at = cfg["auxiliary_training"]
    contract = {
        "format": "bilinear_lmmd.at_sbn.run_contract.v1",
        "protocol": PROTOCOL,
        "scientific_scope": "post-primary exploratory cross-view transfer",
        "fold": int(fold),
        "seed": 42,
        ...
        "objective": {
            ...
            "aux_ce_weight_each": float(at["aux_ce_weight_each"]),
            "self_distill_weight_each": float(at["self_distill_weight_each"]),
            "self_distill_temperature": float(at["self_distill_temperature"]),
            "self_distill_direction": "R0_detached_to_each_auxiliary",
            "merge_weight": float(at["merge_weight"]),
            "merge_start_epoch": int(at["merge_start_epoch"]),
        },
        "auxiliary_dropout": bool(at["auxiliary_dropout"]),
    ...
    if contract_path.is_file():
        if _json(contract_path, "Existing contract") != contract:
            raise RuntimeError("Existing AT-SBN run contract berbeda.")
    else:
    ...
    with exclusive_training_lock(
        output_root,
        lock_name=f"AT_SBN_ALL4_fold{fold}_seed42.training.lock",
        stale_seconds=900,
    ):
    ...
    if not _run_complete(run_dir, int(cfg["training"]["epochs"])):
        raise RuntimeError("Run AT-SBN belum selesai.")
    ...
    result = {
        "format": "bilinear_lmmd.at_sbn.result.v1",
        "protocol": PROTOCOL,
        "method": "AT_SBN_ALL4",
        "fold": int(fold),
        "seed": 42,
