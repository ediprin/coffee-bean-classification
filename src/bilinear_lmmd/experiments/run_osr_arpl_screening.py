...
TIERS = ("near", "medium")
BASELINE_SCORE = "msp"
CANDIDATE_SCORE = "mls"  # max ARPL logits, as in the official evaluation
MIN_AUROC_GAIN = 0.02
MIN_OSCR_GAIN = 0.0
MAX_KNOWN_F1_DROP = 0.01


def compare_arpl_to_gap(baseline: dict, candidate: dict) -> dict:
    """Compare official ARPL max-logit scoring to the frozen GAP+MSP baseline."""
    ...
    return {
        "method": "ARPLoss without confusing-sample GAN",
        "baseline_score": BASELINE_SCORE,
        "candidate_score": "maximum ARPL logit (MLS)",
        "gate": {
        ...
        "next_step": (
            ...
            if passed_tiers
            else "stop ARPL-no-CS on Coffee17 OSR"
        ),
    }


def run_osr_arpl_screening(
    data_root: Path,
    prepared_root: Path,
    output_root: Path,
    protocol_path: Path,
    ...
    for index, tier in enumerate(TIERS, start=1):
        print(
            f"\n=== ARPL-NO-CS {tier.upper()} ({index}/{len(TIERS)}) "
            f"| SEED {seed} ===",
            flush=True,
        )
        tier_root = prepared_root / tier
        run_root = output_root / "outputs" / f"OSR2_{tier}_seed{seed}"
        checkpoint = run_root / "best.pt"
        resolved_config = (
            output_root / "resolved_configs" / f"OSR2_{tier}_seed{seed}.yaml"
        )
            ...
            if artifact_repo:
                command.extend(
                    [
                        ...
                        artifact_repo,
                        "--artifact-path",
                        f"osr-v1/OSR2_{tier}_seed{seed}",
                        "--artifact-sync-every",
        ...
        reports[tier] = evaluate_split(
            ...
            tier_root,
            protocol,
            output_root / "reports" / f"OSR2_{tier}_seed{seed}",
        )

    candidate = {
        "protocol_id": protocol["protocol_id"],
        "seed": seed,
        "model": "EfficientNetV2-B0 + GAP + ARPLoss (no CS)",
        "splits": reports,
    }
    candidate_path = output_root / "reports" / f"osr2_seed{seed}_summary.json"
    candidate_path.write_text(json.dumps(candidate, indent=2), encoding="utf-8")
    result = compare_arpl_to_gap(baseline, candidate)
    result.update(
        {
            "seed": seed,
            "baseline": "EfficientNetV2-B0 + GAP + CE + MSP",
            "candidate": "EfficientNetV2-B0 + GAP + ARPLoss (no CS)",
        }
    )
    result_path = output_root / "reports" / f"osr_arpl_failfast_seed{seed}.json"
    result_path.write_text(json.dumps(result, indent=2), encoding="utf-8")

    print("\n=== KEPUTUSAN ARPL-NO-CS FAIL-FAST ===", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fail-fast official ARPLoss (no CS) on Coffee17 OSR"
    )
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/osr/OSR2_efficientnetv2_arpl_no_cs.yaml"),
    )
