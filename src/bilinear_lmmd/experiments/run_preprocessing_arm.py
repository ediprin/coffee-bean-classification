...
def run_arm(
    *,
    ...
    if seed != 42:
        raise ValueError("Primary/smoke preprocessing contract awal dikunci seed 42.")
    if not required_commit.strip():
        raise RuntimeError("required_commit wajib diisi dengan commit branch eksperimen.")
    ...
    if development["fold"] != fold:
        raise RuntimeError(
            f"Development fold {development['fold']} tidak cocok dengan --fold {fold}."
        )
    static = validate_static_and_equivalence(static_preflight, arm, equivalence)
    ...
    frozen_config_text = yaml.safe_dump(cfg, sort_keys=False, allow_unicode=True)
    frozen_config_sha = canonical_json_sha256(cfg)

    contract = {
        "format": "bilinear_lmmd.preprocessing.arm_contract.v1",
        "protocol": (
        ...
        "development_contract_sha256": development["development_contract_sha256"],
        "static_preflight_sha256": static["static_preflight_sha256"],
        "reference_equivalence_sha256": static["reference_equivalence_sha256"],
        "environment_reference_sha256": environment_gate["environment_reference_sha256"],
    ...
    if contract_path.is_file():
        ...
        if old != contract:
            raise RuntimeError("Existing run directory memiliki run-contract berbeda.")
    else:
        frozen_config_path.write_text(frozen_config_text, encoding="utf-8")
    ...
    result = {
        "format": "bilinear_lmmd.preprocessing.arm_result.v1",
