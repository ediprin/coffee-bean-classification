...
def run_auxce_control(
    *,
    ...
    if fd["method"] != "auxce_sbn_control":
        raise RuntimeError("Config bukan AUXCE-SBN control.")
    if float(fd["feature_distill_weight"]) != 0.0:
        raise RuntimeError("AUXCE-SBN control harus lambda_feat=0.")
    ...
    if wrapped_probe.primary_initial_sha256 != expected_initial:
        raise RuntimeError("AUXCE-SBN wrapper mengubah primary initialization.")
    del wrapped_probe
    ...
    contract = {
        "format": "bilinear_lmmd.auxce_sbn_control.run_contract.v1",
        "protocol": PROTOCOL,
        "scientific_scope": "post-primary exploratory causal ablation",
        "causal_question": (
            "Does MVFD-SBN's gain require the explicit transformed-view "
            "feature-distillation term beyond auxiliary-head CE regularization?"
        ),
        "fold": int(fold),
        "seed": 42,
        ...
        "objective": {
            ...
            "feature_distill_loss_computed_for_diagnostics_only": True,
        },
    ...
    if contract_path.is_file():
        if _json(contract_path, "Existing contract") != contract:
            raise RuntimeError("Existing AUXCE-SBN contract berbeda.")
    else:
    ...
    with exclusive_training_lock(
        output_root,
        lock_name=f"AUXCE_SBN_CONTROL_fold{fold}_seed42.training.lock",
        stale_seconds=900,
    ):
    ...
    if not _run_complete(run_dir, int(cfg["training"]["epochs"])):
        raise RuntimeError("AUXCE-SBN control belum selesai.")
    ...
    result = {
        "format": "bilinear_lmmd.auxce_sbn_control.result.v1",
        "protocol": PROTOCOL,
        "method": "AUXCE_SBN_CONTROL",
        "fold": int(fold),
        "seed": 42,
