...
Detail asumsi dan batas klaim ada di `docs/protocols/PAPER_REPRODUCTION_PROTOCOL.md`.
...
Runner `python -m bilinear_lmmd.experiments.run_final_hbp_report` menghasilkan:
...
Runner `python -m bilinear_lmmd.experiments.run_final_hbp_xai` membandingkan M0 dan M1 pada
